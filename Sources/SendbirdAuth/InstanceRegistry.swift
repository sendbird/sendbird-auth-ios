...
internal final class InstanceRegistry {
    ...
    func createKey(appId: String, apiHostUrl: String?) -> String {
    ...
    func getOrCreate(
        ...
        let key = createKey(appId: params.applicationId, apiHostUrl: params.customAPIHost) as NSString
    ...
    func get(appId: String, apiHostUrl: String? = nil) -> SendbirdAuthMain? {
        let key = createKey(appId: appId, apiHostUrl: apiHostUrl) as NSString
        return lock.withLock {
    ...
    func update(_ instance: SendbirdAuthMain) {
        let key = createKey(appId: instance.applicationId, apiHostUrl: instance.routerConfig.apiHost) as NSString
        lock.withLock {
