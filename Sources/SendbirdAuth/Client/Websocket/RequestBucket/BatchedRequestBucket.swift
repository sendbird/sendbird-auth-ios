...
package actor BatchedRequestBucket {
    package let commandType: CommandType
    package var pendingRequestsCount: Int {
        requestIdStack.count
    }
    package var isFlushInProgress: Bool {
        flushingTask != nil
    }
    ...
    #if TESTCASE
    package init(commandType: CommandType, dispatcher: some WebSocketRequestSendable) {
        self.commandType = commandType
        self.sender = dispatcher
    }
    #endif
    
    package nonisolated func shouldHold(_ request: some WSRequestable) -> Bool {
        request.commandType == self.commandType
    }
    
    package func shouldFlush(_ command: some Command) -> Bool {
        guard
    ...
    package func hold(_ request: some ResultableWSRequest) async {
        guard
            shouldHold(request),
    ...
    package func flushPendingRequests(with command: some Command, _ flushHandler: @escaping (Command) async -> Void) async {
        await waitForFlush()
