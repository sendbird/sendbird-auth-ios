...
public protocol SessionExpirable {
...
extension SessionManager: InternalSessionDelegate {
    public func didSessionTokenFailToRefresh(error: AuthClientError) {
        logout()
    ...
    public func didSessionKeyFailToRefresh(error: AuthClientError) {
        sessionHandler.didHaveError(error.asAuthError)
        router.eventDispatcher.dispatch(command: SessionExpirationEvent.RefreshFailed())
    }
    
    public func didSessionKeyRefresh(key: Session, requireReconnect: Bool) {
    ...
    public func didSessionTokenRevoke() {
    ...
    public func refreshSessionKey(authToken: String?, expiringSession: Bool, expiresIn: Int64?, completionHandler: ((Bool, Session?, AuthError?) -> Void)?) {
