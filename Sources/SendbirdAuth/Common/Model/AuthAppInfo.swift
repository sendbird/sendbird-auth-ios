...
package final class AuthAppInfo: NSObject, Codable {
    ...
    /// - Since: 4.9.5
    package var uploadSizeLimitInMB: Int64 = 0

    /// This is the premium feature list using on your Application ID.
    /// - Since: 3.0.180
    package var premiumFeatureList: [String]?

    /// This is the state of using the reaction feature.
    /// - Since: 3.0.180
    package var useReaction: Bool

    /// This is the application attributes list using on your Application ID.
    /// - Since: 3.0.198
    package var applicationAttributes: [String]?

    /// This is the application attribute to tell SDK use native websocket
    /// - Since: 3.0.222
    package var useNativeWS: Bool = false

    ///
    /// - Since: 3.0.231
    package var concurrentCallLimit: Int = 0

    ///
    /// - Since: 3.0.231
    package var backOffDelay: TimeInterval = 0.0
    
    /// A getter for the `disableSupergroupMACK` property.
    /// - Since: 3.0.230
    package var disableSuperGroupMACK: Bool
    ...
    package var typedApplicationAttributes: Set<ApplicationAttribute> {
        Set(applicationAttributes?.compactMap { ApplicationAttribute(rawValue: $0) } ?? [])
    }
    
    /// - Since: 4.6.0
    @InternalAtomic package var notificationInfo: AuthNotificationInfo?
    
    /// - Since: 4.17.0
    @InternalAtomic package var messageTemplateInfo: AuthMessageTemplateInfo?
    
    /// - Since: 4.26.0
    @InternalAtomic package var aiAgentInfo: AuthAIAgent.Info
    
    /// - Since: 4.8.4
    package let uikitConfigInfo: AuthUIKitConfigInfo
    
    /// The maximum number of files that can be sent in a `MultipleFilesMessage`.
    /// - Since: 4.9.1
    package let multipleFilesMessageFileCountLimit: Int

    /// - Since: 4.17.0
    package var isChannelMembershipHistoryEnabled: Bool {
        guard let applicationAttributes else { return false }
        return applicationAttributes.contains("channel_membership_history")
    }
    
    /// - Since: 4.17.0
    package var isLeftUserViewSupportEnabled: Bool {
        guard let applicationAttributes else { return false }
        return applicationAttributes.contains("left_user_view_support")
    }
    
    /// - Since: 4.18.0
    package let statsConfig: SafeDictionary<String, StatConfig>?
    
    /// StatConfig가 도입되기 전에 하드코딩 되어있던 설정
    /// - Since: 4.18.0
    package var defaultConfig: StatConfig? {
        statsConfig?[StatConfigKeys.default.rawValue]
    }
    
    /// Notification 로그를 위한 설정
    /// - Since: 4.18.0
    package var notificationConfig: StatConfig? {
    ...
    package enum ApplicationAttribute: String {
        ...
        package var isRelatedToStat: Bool {
            switch self {
    ...
    package enum StatConfigKeys: String {
     ...
     */
    @objc
    package func isEmojiUpdateNeeded(prevEmojiHash: String) -> Bool {
