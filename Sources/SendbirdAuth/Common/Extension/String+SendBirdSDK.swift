...
@_spi(SendbirdInternal) public extension String {
    ...
    var pathExtension: String? {
        guard contains("."), let ext = self.split(separator: ".").last else { return nil }
        return String(ext)
    }
    ...
    static func isEqualJsonString(_ lhs: String, _ rhs: String) -> Bool {
        guard let data1 = lhs.data(using: .utf8),
              let data2 = rhs.data(using: .utf8) else {
            return false
        }
...
//    }
    ...
    @_spi(SendbirdInternal) public var urlEncoded: String {
        ...
        guard let encoded = self.addingPercentEncoding(withAllowedCharacters: allowed) else {
            return self
        }
